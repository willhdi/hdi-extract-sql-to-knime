-- Nodo KNIME : P&G_COCO\PREMIUM EARN (#298)\DB SQL Executor (#356)
-- Clave      : statement
...
CASE WHEN b.documento IS NULL THEN A.RECIBO ELSE b.documento END AS DOC,
CASE WHEN b.documento IS NULL THEN a.VALOR_CONCEPTO
	 ELSE a.VALOR_CONCEPTO * (b.PARTICIPACION/100) 
END as VALOR_CONCEPTO_CO,
b.COD_SUCURSAL
into #caso1
from  #si_coco a
left join #corretaje b on (concat(ltrim(rtrim(A.RAMO_PROD)),'_',ltrim(rtrim(A.poliza)),'_',A.certificado)=B.LLAVE_CERT) --AND A.documento>=B.documento and A.documento<B.doc_2)
where b.PARTICIPACION is not null
